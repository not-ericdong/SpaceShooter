from PIL import Image, ImageColor, ImageDraw
import random
import os
import time
import pygame

# pygame setup
pygame.font.init()
WIDTH, HEIGHT = 600, 900
WINDOW = pygame.display.set_mode((WIDTH, HEIGHT))
pygame.display.set_caption("APOLLO ADVENTURE")

# background generator
assets = os.path.dirname(__file__)
bg_filename = os.path.join(assets, 'background.jpg')

space = Image.new("RGB", (WIDTH, HEIGHT), "black")
d = ImageDraw.Draw(space)

star_number = random.randrange(200, 250, 5)
colour_list = ["#4291E5", "#70ABE9", "#B8D3EF",
               "#FEF8EC", "#FDE4B2", "#E9C57D", "#F5AC1B"]

# stars can be though of as 5 squares of equal size organized into a cross
# offset is how far away from origin (top left corner)
for x in range(star_number):
    offsetx = random.randrange(0, WIDTH, 1)
    offsety = random.randrange(0, HEIGHT, 1)
    colour = random.choices(colour_list, weights=(3, 3, 5, 3, 2, 2, 6))[0]
    # more accurate weight distribution: weights=(0.0000003, 0.0013, 0.006, 0.03, 0.076, 0.121, 0.7645)
    size = random.randrange(1, 3, 1)

    if space.getpixel((offsetx, offsety)) is not (0, 0, 0):
        star_x = d.rectangle([offsetx, size + offsety,
                              3 * size + offsetx, 2 * size + offsety], fill=colour)
        star_y = d.rectangle([size + offsetx, offsety,
                              2 * size + offsetx, 3 * size + offsety], fill=colour)
space.save(bg_filename)

# Load the images
SPACESHIP = pygame.image.load(os.path.join(assets, "spaceship_s.png"))
UFO1_S = pygame.image.load(os.path.join(assets, "ufo_blue.png"))
UFO2_S = pygame.image.load(os.path.join(assets, "ufo_pink.png"))
UFO3_S = pygame.image.load(os.path.join(assets, "ufo_purple.png"))
UFO4_S = pygame.image.load(os.path.join(assets, "ufo_orange.png"))  # _S images are actually bigger im too lazy..
UFO1 = pygame.image.load(os.path.join(assets, "ufo_blue_s.png"))
UFO2 = pygame.image.load(os.path.join(assets, "ufo_pink_s.png"))
UFO3 = pygame.image.load(os.path.join(assets, "ufo_purple_s.png"))
UFO4 = pygame.image.load(os.path.join(assets, "ufo_orange_s.png"))

PROJECTILE = pygame.image.load(os.path.join(assets, "tennis_ball_s.png"))
PROJECTILE_BAD = pygame.image.load(os.path.join(assets, "tennis_ball_red_s.png"))
POWERUP_BONE = pygame.image.load(os.path.join(assets, "bone.png"))
POWERUP_HEART = pygame.image.load(os.path.join(assets, "heart.png"))
BACKGROUND = pygame.image.load(os.path.join(assets, "background.jpg"))


class Projectile:
    def __init__(self, x, y, img, projectile_list=[]):
        self.x = x
        self.y = y
        self.projectile_img = img
        self.projectile_list = projectile_list
        self.mask = pygame.mask.from_surface(self.projectile_img)

    def draw(self, window):
        window.blit(self.projectile_img, (self.x, self.y))

    def move(self, speed):
        self.y += speed

    def offscreen(self, height):
        return not (height >= self.y >= 0)

    def collision(self, obj):
        return collide(self, obj)


def collide(obj1, obj2):
    offset_x = obj2.x - obj1.x
    offset_y = obj2.y - obj1.y
    return obj1.mask.overlap(obj2.mask, (int(offset_x), int(offset_y))) is not None


class Ship:
    COOLDOWN = 30

    def __init__(self, x, y, health=50):
        self.x = x
        self.y = y
        self.health = health
        self.ship_img = None
        self.cool_down = 0
        self.projectile_img = PROJECTILE_BAD
        self.projectiles = []

    def cooldown(self):
        if self.cool_down >= self.COOLDOWN:
            self.cool_down = 0
        elif self.cool_down > 0:
            self.cool_down += 1

    def shoot(self):
        if self.cool_down == 0:
            projectile = Projectile(self.x + self.get_width()/2 - 6,  # need to subtract the width of the projectile
                                    self.y, self.projectile_img)
            self.projectiles.append(projectile)
            self.cool_down = 1

    def draw(self, window):
        window.blit(self.ship_img, (self.x, self.y))
        for projectile in self.projectiles:
            projectile.draw(window)

    def get_width(self):
        return self.ship_img.get_width()

    def get_height(self):
        return self.ship_img.get_height()


class Player1(Ship):
    def __init__(self, x, y, health=50):
        super().__init__(x, y, health)
        self.ship_img = SPACESHIP
        self.projectile_img = PROJECTILE
        self.mask = pygame.mask.from_surface(self.ship_img)
        self.max_health = health
        self.score = 0

    def draw(self, window):
        super().draw(window)
        self.health_bar(window)

    def health_bar(self, window):
        pygame.draw.rect(window, (255, 0, 0), (self.x, self.y + self.ship_img.get_height() + 10,
                                               self.ship_img.get_width(), 10))
        pygame.draw.rect(window, (0, 255, 0), (self.x, self.y + self.ship_img.get_height() + 10,
                                               (self.health/self.max_health) * self.ship_img.get_width(), 10))
        # todo still a bit of green left after no health remove this


class EnemyShip(Ship):
    colour_picker = {
                    "purple": UFO3,
                    "blue": UFO1,
                    "orange": UFO4,
                    "pink": UFO2,
                    "purple_s": UFO3_S,
                    "blue_s": UFO1_S,
                    "orange_s": UFO4_S,
                    "pink_s": UFO2_S
                    }

    def __init__(self, x, y, colour, health=50):
        super().__init__(x, y, health)
        self.ship_img = self.colour_picker[colour]
        self.mask = pygame.mask.from_surface(self.ship_img)
        self.projectile_img = PROJECTILE_BAD

    def shoot(self):
        if self.cool_down == 0:
            projectile = Projectile(self.x + self.get_width()/2,  # need to subtract the width of the projectile
                                    self.y + self.get_height(), self.projectile_img)
            self.projectiles.append(projectile)
            self.cool_down = 1

    def move(self, vel):  # todo add x movement random
        self.y += vel


def main():
    run = True
    fps = 60
    clock = pygame.time.Clock()
    level = 0
    lives = 3
    main_font = pygame.font.SysFont("Arial", 20)
    lost_font = pygame.font.SysFont("Arial", 50)
    lost = False
    lost_count = 0

    player_speed = 8
    player = Player1(WIDTH/2,
                     HEIGHT - 100)
    score = player.score
    proj_speed = 10
    proj_speed_bad = min(5 + level, 15)
    powerup_speed = 7
    powerup_counter1 = 0

    enemy_list = []
    wave_length = 5
    enemy_speed = 1

    def redraw_window():
        WINDOW.blit(BACKGROUND, (0, 0))

        # Ships
        player.draw(WINDOW)
        for enemy in enemy_list:
            enemy.draw(WINDOW)

        # UI
        level_display = main_font.render(f"Level: {level}", 1, (255, 255, 255))
        lives_display = main_font.render(f"Lives: {lives}", 1, (255, 255, 255))
        score_display = main_font.render(f"Score: {score}", 1, (255, 255, 255))
        WINDOW.blit(lives_display, (20, 20))
        WINDOW.blit(score_display, (20, HEIGHT - 50))
        WINDOW.blit(level_display, (WIDTH - level_display.get_width() - 20, 20))
        if lost:
            lost_title = lost_font.render('YOU LOSE', 1, (255, 255, 255))
            WINDOW.blit(lost_title, (WIDTH/2 - lost_title.get_width()/2,
                                     HEIGHT/2 - lost_title.get_height()/2))
            WINDOW.blit(score_display, (WIDTH/2 - score_display.get_width()/2,
                                        HEIGHT/2 - score_display.get_height()/2 + 50))
            # todo remove the windowblit of live score and stuff maybe

        pygame.display.update()

    #  ---------------MAIN LOOP---------------
    while run:
        clock.tick(fps)
        redraw_window()
        score = player.score

        if lives <= 0 or player.health <= 0:
            lost = True
            lost_count += 1
        if lost:
            if lost_count > fps*3:
                run = False
            else:
                continue

        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                quit()

        # Power ups





        # Movement of player ship
        keys = pygame.key.get_pressed()
        if keys[pygame.K_w] and player.y - player_speed > 0:  # up
            player.y -= player_speed
        if keys[pygame.K_a] and player.x - player_speed > 0:  # left
            player.x -= 1.2*player_speed
        if keys[pygame.K_s] and player.y + player_speed + player.get_height() < HEIGHT:  # down
            player.y += player_speed
        if keys[pygame.K_d] and player.x + player_speed + player.get_width() < WIDTH:  # right
            player.x += 1.2*player_speed
        if keys[pygame.K_SPACE]:
            player.shoot()

        # ship spawning - balance at the end
        if len(enemy_list) == 0:
            level += 1
            wave_length += 5
            for n in range(wave_length):
                enemy = EnemyShip(random.randrange(10, WIDTH - 200),
                                  random.randrange(-1000, -100),
                                  random.choice(["purple", "blue", "pink", "orange",
                                                 "purple_s", "blue_s", "pink_s", "orange_s"])
                                  )
                enemy_list.append(enemy)
        # enemy
        for enemy in enemy_list:
            enemy.move(enemy_speed)
            enemy_projectile_list = []
            for x in enemy.projectiles:
                enemy_projectile_list.append(x)

            # enemy.projectile_movement(proj_speed_bad, player) #this ties the projectile with the enemy
            # todo change this so projectile is not bound by enemy

            """-----------------"""

            enemy.cooldown()
            for projectile in enemy_projectile_list:
                projectile.move(proj_speed_bad)
                if projectile.offscreen(HEIGHT):
                    enemy_projectile_list.remove(projectile)
                elif projectile.collision(player):
                    player.health -= 10
                    enemy_projectile_list.remove(projectile)
            """-----------------"""

            if random.randrange(0, 2 * fps) == 1:  # for quicker testing
                # if random.randrange(0, min(3,(11-level))*fps) == 1:
                enemy.shoot()
                """-----------------"""
            if collide(enemy, player):
                player.health -= 10
                enemy_list.remove(enemy)  # this removes the projectile too
            elif enemy.y + enemy.get_height() > HEIGHT:
                lives -= 1
                enemy_list.remove(enemy)

        #player bullets
        player.cooldown()
        player_projectiles = player.projectiles
        for projectile in player_projectiles:
            projectile.move(-proj_speed)
            if projectile.offscreen(HEIGHT):
                player_projectiles.remove(projectile)
            else:
                for ufo in enemy_list:
                    if projectile.collision(ufo):
                        player.score += 1
                        enemy_list.remove(ufo)  # removes ufo after it gets hit with player projectile
                        if projectile in player_projectiles:
                            player_projectiles.remove(projectile)  # removes player projectile after hitting ufo




def main_menu():
    run = True
    title_font = pygame.font.SysFont("Arial", 50)
    while run:
        WINDOW.blit(BACKGROUND, (0, 0))
        title = title_font.render("Press the mouse to begin!", 1, (255, 255, 255))
        WINDOW.blit(title, (WIDTH/2 - title.get_width()/2, 350))
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
            if event.type == pygame.MOUSEBUTTONDOWN:
                main()
    pygame.quit()


main_menu()
